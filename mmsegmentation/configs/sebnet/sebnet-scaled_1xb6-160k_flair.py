...
imagenet_checkpoint_file = "checkpoints/imagenet/epoch_98.pth"
...
model = dict(
    ...
    backbone=dict(
        type='SEBNet', # The type of the backbone module. All fields except `type` come from the __init__ method of class `SEBNet_Staged`
        arch='xlarge',
        in_channels = 3,
        align_corners = False,
        init_cfg=dict(type='Pretrained', checkpoint=imagenet_checkpoint_file)),
    neck=dict(
        type='DAPPMScaled', # The type of the neck module.
        arch='xlarge_scaled',
        num_scales=5),    
    decode_head=dict(
        type='PIDNetSBDHead', # The type of the segmentation decode head module.
        arch='xlarge_scaled',
        num_classes=19,
        eval_edges=False,
        loss_decode=[
