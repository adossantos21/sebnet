"""
This script was developed based on https://github.com/nv-tlabs/GSCNN/blob/master/utils/f_boundary.py
All credit goes to the authors of Gated-SCNN.
-----------------------------------------------------------------------------------------------------
...
class BFScore:
    def __init__(
        ...
        self.px_tolerance = px_tolerance
    ...
    def forward(self):
        ...
        for pred, mask in tqdm(zip(self.preds, self.masks), desc="Processing BFScores", total=len(self.preds)):
            ...
            assert seg_mask.shape == gt_mask.shape, f"prediction and ground truth should have the same shape; Instead, prediction '{pred}' shape is '{seg_mask.shape}' and ground truth '{mask}' shape is '{gt_mask.shape}'."
            self.bd_thresh = round(float(self.px_tolerance / sqrt((gt_mask.shape[1]**2)+(gt_mask.shape[2]**2))), 4)
