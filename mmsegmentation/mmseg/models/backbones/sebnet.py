...
class SEBNet(BaseBackbone):
    ...
    def __init__(self,
        ...
        assert (isinstance(self.depths, Sequence)
                ...
                and len(self.depths) == len(self.channels) == 5), \
            f'The "depths" ({self.depths}) and "channels" ({self.channels}) ' \
            f'should both be sequences of length 5.'
        ...
        for i in range(self.num_stages):
            ...
            if i == 0:
                ...
                self.stages.append(
                    self._make_layer(
                        block=BasicBlock if i < 4 else Bottleneck,
                        in_channels=self.channels[i - 1],
                        channels=channels if i < 4 else channels // 2,
                        depth=depth,
                        stride=2))
