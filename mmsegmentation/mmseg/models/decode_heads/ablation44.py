...
from mmseg.models.utils import (
    ...
    EdgeModuleConditioned as SBDModule,
    EdgeModuleFused as HEDModule,
    Bag as PreHead
)
...
class Ablation44(BaseDecodeHead):
    """
    Ablation 44 - Baseline + P Head (Fused) + Edge Head (Fused, HED)
    + Edge Head (Conditioned, SBD), supervised with HED and SBD 
    supervisory signals. Fusion present for P Head and Edge Head. 
    See Holistically-Nested Edge Detection (HED) at 
    https://arxiv.org/pdf/1504.06375.pdf and Semantic Boundary 
    ...
    def forward(self, x: Tuple[Tensor, ...]):
        ...
        if self.training:
            ...
            x_hed, x_d = self.hed_module(x) # x_hed: (N, 128, H/8, W/8), x_d: (N, 256, H/8, W/8)
