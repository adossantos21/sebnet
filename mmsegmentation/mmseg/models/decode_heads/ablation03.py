...
class Ablation03(BaseDecodeHead):
    """
    Ablation 03 - Baseline + P Head, with Pag1 supervised via 
    ...
    def forward(self, x: Tuple[Tensor, ...]):
        ...
        if self.training:
            x_p = self.p_module(x)
            p_supervised = self.p_head(x_p[0], self.p_cls_seg) # (N, K, H/8, W/8), where K is the number of classes in the labeled dataset
            x[-1] = F.interpolate(
                x[-1],
                size=x[1].shape[2:],
                mode='bilinear',
