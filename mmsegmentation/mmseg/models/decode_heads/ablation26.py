...
class Ablation26(BaseDecodeHead):
    """
    Ablation 26 - Baseline + P Head (Conditioned) + Edge Head (Conditioned),
    supervised with a SBD signal. No fusion. See Semantic Boundary Detection 
    (SBD) at https://arxiv.org/pdf/1705.09759 for more details. 
    ...
    def forward(self, x: Tuple[Tensor, ...]):
        ...
        """
        if self.training:
            x_p_feats = self.p_module(x) # temp_p: (N, 128, H/8, W/8), x_p: (N, 256, H/8, W/8)
            x_edges = self.edge_module(x) # temp_d: (N, 128, H/8, W/8), x_d: (N, 256, H/8, W/8)
            x[-1] = F.interpolate(
                x[-1],
                size=x[1].shape[2:],
                mode='bilinear',
            ...
            sbd = self.sbd_head(x_edges, self.sbd_cls_seg) # (N, 1, H/8, W/8)
