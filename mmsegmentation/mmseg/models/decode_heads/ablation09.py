...
class Ablation09(BaseDecodeHead):
    """
    Ablation 09 - Baseline + BEM Head, conditioned with two SBD
    ...
    def forward(self, x: Tuple[Tensor]):
        ...
        """
        if self.training:
            side5, fuse = self.edge_module(x) # side5 and fuse (N, C=128, H/8, W/8)
            x[-1] = F.interpolate(
                x[-1],
                size=x[1].shape[2:],
                mode='bilinear',
                align_corners=self.align_corners)
            side5 = self.side5_head(side5, self.side5_cls_seg) # (N, K, H/8, W/8), where K is the number of classes in the labeled dataset
            fuse = self.fuse_head(fuse, self.fuse_cls_seg) # (N, K, H/8, W/8)
