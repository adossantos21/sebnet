# SEBNet with one bottleneck followed by a basic block, as opposed to two bottlenecks, + FCN Multi-Loss
...
class SEBNetTest8(BaseBackbone):
    """SEBNet backbone.
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(SEBNetTest8, self).__init__(init_cfg)
        ...
        self.i_branch_layers.append(
            nn.Sequential(
                self._make_layer(Bottleneck, channels * 8, channels * 8, 1, stride=2),
                self._make_layer(BasicBlock, channels * 8 * 4, channels * 8 * 4, 1)
            )
        )
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        ...
        x_5_1 = self.i_branch_layers[2][1](x_5_0)
