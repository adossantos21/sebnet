...
class PIDNet(BaseModule):
    ...
    def _freeze_backbone(self):
        ...
        This method freezes all the backbone components while leaving
        any subsequently added neck/head layers trainable.
        """
        # Freeze stem layer
        ...
        # Freeze all I, P, D branch layers
        for branch_name in ['i_branch_layers', 'p_branch_layers', 'd_branch_layers']:
        ...
        # Freeze compression layers
        for comp_name in ['compression_1', 'compression_2']:
            if hasattr(self, comp_name):
                comp_layer = getattr(self, comp_name)
                for param in comp_layer.parameters():
                    param.requires_grad = False
                comp_layer.eval()
        
        # Freeze PAG (Pixel-attention-guided fusion) modules
        for pag_name in ['pag_1', 'pag_2']:
            if hasattr(self, pag_name):
                pag_layer = getattr(self, pag_name)
                for param in pag_layer.parameters():
                    param.requires_grad = False
                pag_layer.eval()
        
        # Freeze diff layers
        for diff_name in ['diff_1', 'diff_2']:
            if hasattr(self, diff_name):
                diff_layer = getattr(self, diff_name)
                for param in diff_layer.parameters():
                    param.requires_grad = False
                diff_layer.eval()
    ...
    def train(self, mode=True):
        ...
        # Re-freeze backbone components if they should remain frozen
        if hasattr(self, 'freeze_backbone') and self.freeze_backbone:
            self._freeze_backbone()
